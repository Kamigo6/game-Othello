...
#in ban co trong qua trinh choi
#print 
...
def isFlippable(cur_state, row, col, player):
    ...
    for pos in SURROUND_POSITION:
        ...
        if currentRow > 7 or currentRow < 0 or currentCol > 7 or currentCol < 0:
            ...
            currentPiece = cur_state[currentRow][currentCol]  #current state : BLACK or WHITE
        ...
        if (player == BLACK):
            if (not possibleMoves):
                ...
                if black_ai == 1:
                    ...
                    start = time.time()
                    computerMove = localMaximum(BOARD, player, SCORE, possibleMoves)
                    end = time.time()
                    print("Black time: ", end - start)
                elif black_ai == 3:
            ...
            if (not possibleMoves):
                ...
                if white_ai == 1:
                    ...
                    print("White time minimax: ", end - start)
                    start = time.time()
                    computerMove = alphaBeta(BOARD, player, DEPTH, float("-inf"), float("inf"), True)[2]
                    end = time.time()
                    print("White time AB: ", end - start)
                elif white_ai == 4:
                    computerMove = alphaBeta(BOARD, player, DEPTH, float("-inf"), float("inf"), True)[2]
                elif white_ai == 5:
                    computerMove = negamax(BOARD, player, DEPTH)[2]
...
# usage for white - AI:
# minimaxResult = minimax(BOARD, player=AI, depth=4, maximizing)
